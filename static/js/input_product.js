...
$(document).on("click", '.inp5438', function (e) {
    ...
    textValue = parenElem.querySelector('.inp5432_row .col-8 input').value
...
async function func_check(es, e) {
    ...
    const stat_str = await stat_data.then(r => {
        return r
    })
    ...
    await stat_int['data'].forEach(d => {
        ...
        els1.addEventListener('click', function (e) {
            ...
            let elm2 = elm1.querySelectorAll('.modul_select_row select')
...
let hClick3 = (event) => {
    ...
    elem_row_text.forEach(div => {
        i++;
        sas = div.querySelector('.inp5432_row .col-8 input').value
...
const s123234 = document.querySelector('button[name="record_prod_23"]')
s123234.addEventListener('click', function (event) {
    rec_data()
...
function rec_data() {
    const s121 = $("select[name='id_prov_SDS_1']").val()
    if (!s121) {
        error_modal('Выбери поставщика');
        return;
    }
    console.log('ok')
}
